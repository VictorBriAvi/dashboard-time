...
export default function DataTableTanstack({ data }: DataTableTanstackProps) {
  ...
  const columns = useMemo<ColumnDef<ColaboradorRow, any>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (info) => `$${(info.getValue<number>() ?? 0).toLocaleString("es-AR")}`,
      },
      {
  ...
  // Función para manejar la visualización de los servicios realizados
  const handleViewServices = (rowData: ColaboradorRow) => {
    console.log("Servicios realizados por: ", rowData.empleado);
    console.log(rowData.serviciosRealizados);
    // Aquí puedes abrir un modal o alguna otra vista con los servicios
  };
                    ...
                    <td
                      ...
                      className={`px-3 py-2 sm:px-4 sm:py-3 text-xs sm:text-sm whitespace-nowrap ${
                        cell.column.id.includes("porcentaje")
                          ...
                          : cell.column.id.includes("totalVentas") || cell.column.id.includes("totalAPagar")
